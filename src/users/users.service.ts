...
export class UsersService {
  ...
  private saveUser( name: string, email:string, password:string, signupVerfifyToken: string){
    //DB 연동 후 구현 
    return;
  }
  ...
  async verifyEmail(signupVefityToken: string): Promise<string>{

    //TODO
    //1. DB에서 signupVerifyToken으로 회원 가입 처리중인 유저가 있는지 조회하고 없다면 에러 처리
    //2. 바로 로그인 상태가 되도록 JMT 발급
  ...
  async login( email: string, password: string): Promise<string> {
    ...
    throw new Error('Method net implemented');
  }
